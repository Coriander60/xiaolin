...
内核是应用和硬件设备（比如内存、cpu、硬盘等）之间的桥梁，应用程序只需关心与内核交互，不用关心硬件的细节。
...
- 管理进程和线程，决定哪个进程、线程使用 CPU；
- 提供系统调用，它是用户程序与操作系统之间的接口
- 管理内存，决定内存的分配和回收；
- 管理硬件设备，为进程与硬件设备之间提供通信能力；

⚪用户态和内核态是什么？为什么要有用户态和内核态，只有内核态不行吗？内核是怎么工作的？
...
用户空间的程序只能访问一个局部的内存空间，而内核空间的程序可以访问所有内存空间。  
因此，当程序使用用户空间，在**用户态**执行，  
而当程序使内核空间时，在**内核态**执行。
